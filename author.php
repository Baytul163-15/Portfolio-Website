...
<?php
    ...
    if (isset($_GET['author_id'])) {
        ...
        $page = $obj_client->auth_pagination_number($auth_id);
    }

    #header_auth_data
    if (isset($_GET['author_id'])) {
        $auth_id = $_GET['author_id'];
        $post = $obj_client->header_auth_data($auth_id);
    }

    #author_name_show
    if (isset($_GET['author_id'])) {
        $auth_id = $_GET['author_id'];
        $auth_data = $obj_client->authentication_name($auth_id);
    }
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                    <h2 class="page-heading"><?php echo $auth_data['username']; ?></h2>
                    ...
                    <?php
                        ...
                        if ($page_number > 1) {
                            echo '<li><a href="author.php?author_id='.$auth_id.'&page='.($page_number-1).'">pre</a></li>';
                        }
                        for ($i = 1; $i <= $page; $i++) { 
                            if ($i == $page_number) {
                            $active = "active";
                            ...
                            echo '<li class='.$active.'><a href="author.php?author_id='.$auth_id.'&page='.$i.'">'.$i.'</a></li>';
                        }
                        if ($page > $page_number) {
                            echo '<li><a href="author.php?author_id='.$auth_id.'&page='.($page_number+1).'">next</a></li>';
                        }               
                        echo "</ul>";
                    ?>
                </div><!-- /post-container -->
